...
def validate_request(request: Request) -> None:
    ...
    raise exceptions.UnsupportedMediaType(
        "Content type was expected to be application/json"
    )
...
def validate_request_dict(request: Request) -> None:
    ...
    if type(content) == dict:
        return
    raise exceptions.UnsupportedMediaType(
        "Content was expected to be on dictionary format"
    )
...
def validate_datafile_content(content: DataFileContentRequestType) -> None:
    try:
        ...
        raise exceptions.BadRequest(
            "DataFile must have a tag <string>, path <string> and session <string>."
        )
...
def validate_datafile_setup_attempt(content: DataFileSetupRequestType) -> None:
    try:
        ...
        raise exceptions.BadRequest(
            "DataFile setup should declare an encoding <string>, sep <string> and decimal <string>."
        )
    ...
    if type(content) == list:
        if all([type(feature) == dict for feature in content]):
            if all(
                [
                    "datatype" in feature
                    and feature["datatype"] in {"numerical", "non-numerical"}
                    for feature in content
                ]
            ):
                return None
            raise exceptions.BadRequest(
                "Features should declare its name <string> and datatype <string>."
            )
        raise exceptions.BadRequest(
            "Features must be notified through an array containing the features as maps."
        )
        ...
        raise exceptions.BadRequest(
            "Request must contain an array of features or an error."
        )
