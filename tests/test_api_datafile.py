...
def test_api_datafile_upload_empty_payload_content(test_client):
    ...
    response = test_client.post(
        "/datafile/upload", data=dict(), content_type="multipart/form-data"
    )
...
def test_api_datafile_upload_wrong_content_type(test_client):
    ...
    response = test_client.post(
        "/datafile/upload", data=json.dumps(dict()), content_type="application/json"
    )
    assert response.status_code == 400
...
def test_api_datafile_upload(temp_csv, test_client):
    ...
    file = FileStorage(
        stream=open(temp_csv, "rb"), filename="test.csv", content_type="text/csv"
    )
    response = test_client.post(
        "/datafile/upload", data=dict(file=file), content_type="multipart/form-data"
    )
...
def test_api_datafile_post_nonexistent_file(test_client):
    ...
    response = test_client.post(
        "/datafile/",
        data=json.dumps(dict(tag="df_test", path="non_existent.csv")),
        content_type="application/json",
    )
    assert response.status_code == 400
...
def test_api_datafile_post(test_client, file_path, session_id):
    ...
    response = test_client.post(
        "/datafile/",
        data=json.dumps(dict(tag="df_test", path=file_path, session=session_id)),
        content_type="application/json",
    )
    assert response.status_code == 201
...
def test_api_datafile_patch(test_client, datafile_id):
    ...
    response = test_client.patch(
        f"datafile/{datafile_id}",
        data=json.dumps(dict(tag="new_tag")),
        content_type="application/json",
    )
...
def test_api_datafile_attempt_setup(test_client, datafile_id):
    ...
    response = test_client.post(
        f"datafile/{datafile_id}/setup_attempt",
        data=json.dumps(dict(sep=";", decimal=".", encoding="latin-1")),
        content_type="application/json",
    )
    assert response.status_code == 201
...
def test_api_datafile_notify_setup(test_client, datafile_id):
    ...
    response = test_client.post(
        f"datafile/{datafile_id}/setup_notify",
        data=json.dumps(
            [
