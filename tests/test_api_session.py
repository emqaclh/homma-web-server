...
def test_api_session_post(test_client):
  ...
  response = test_client.post('/session/', data=json.dumps(dict(name='test_session_creation')), content_type='application/json')
...
def test_api_session_patch_nonexistent(test_client):
  response = test_client.patch('/session/non_existent_session', data=json.dumps(dict(name='new_name')), content_type='application/json')
  assert response.status_code == 404
...
def test_api_session_patch_empty_payload_content(test_client, session_id):
  ...
  response = test_client.patch(f'/session/{session_id}', data=json.dumps(dict()), content_type='application/json')
  assert response.status_code == 400

def test_api_session_patch_wrong_payload_type(test_client, session_id):

  response = test_client.patch(f'/session/{session_id}', data=json.dumps([]), content_type='application/json')
  assert response.status_code == 415  

def test_api_session_patch(test_client, session_id):

  response = test_client.patch(f'/session/{session_id}', data=json.dumps(dict(name='new_name')), content_type='application/json')
